        ...
        self.0.iter().all(|group| {
            group.iter().any(|prop| {
                ...
                let count = match prop.name.as_str() {
                    ...
                    name if name.starts_with("RG") => {
                        data.rings[name[2..].parse::<usize>().unwrap()]
                    }
                    name if name.starts_with("AR") => {
                        data.aromatic[name[2..].parse::<usize>().unwrap() - 1]
                    }
                    name => data
                        .bonds
                        .iter()
                        .filter(|(_, kind)| kind.has_property(name))
                        .count(),
                };
                if !prop.count.map_or(count > 0, |required| count == required) {
                    return false;
                }
                if prop.primes == 0 {
                    return true;
                ...
                let has_bond = data
                    .bonds
                    .iter()
                    .any(|&(other, kind)| other == parent && kind.has_property(&prop.name));
                // jbond permits DL in a primed db/DB test; bondinfo does not count it as DB.
                let has_bond = has_bond
                    || (matches!(prop.name.as_str(), "db" | "DB")
...
fn element_matches(
    ...
    if name == "EW" {
        return super::is_elec_withdrawing_element(atoms[idx].element);
    }
    if let Some(group) = wild.iter().find(|group| group.name == name) {
        return group.elements.iter().any(|member| {
            ...
            member[..split] == atoms[idx].element.to_letter()
                && (split == member.len()
                    || member[split..].parse::<usize>().ok() == Some(env[idx].degree))
        });
    }
...
fn match_pending(
    pending: &[(usize, &Neighbor)],
    used: &mut [bool],
    atoms: &[AtomGeneric],
    env: &[AtomEnvData],
    wild: &[WildAtom],
) -> bool {
    let Some((&(parent, pattern), rest)) = pending.split_first() else {
        return true;
    };
    for &(idx, _) in &env[parent].bonds {
        if used[idx]
            || pattern
                .degree
                .is_some_and(|degree| degree != env[idx].degree)
            || !element_matches(&pattern.element, idx, atoms, env, wild)
            || !pattern.properties.matches(idx, Some(parent), env)
        {
            continue;
        }
        used[idx] = true;
        ...
        if match_pending(&next, used, atoms, env, wild) {
            return true;
        }
        used[idx] = false;
    }
...
impl Parser<'_> {
    ...
    fn group(&mut self) -> Option<Vec<Neighbor>> {
        ...
        loop {
            let start = self.pos;
            while self.text.get(self.pos).is_some_and(u8::is_ascii_alphabetic) {
                self.pos += 1;
            }
            if start == self.pos {
                return None;
            }
            let element = std::str::from_utf8(&self.text[start..self.pos])
                .ok()?
                .to_owned();
            let start = self.pos;
            ...
            let properties = if self.text.get(self.pos) == Some(&b'[') {
                let start = self.pos;
                while self.text.get(self.pos).is_some_and(|&ch| ch != b']') {
                    self.pos += 1;
                }
                if !self.take(b']') {
                    return None;
            ...
            let children = if self.text.get(self.pos) == Some(&b'(') {
                self.group()?
            } else {
                Vec::new()
            };
