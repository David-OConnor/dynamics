...
use bio_files::{AtomGeneric, md_params::{DihedralParams, ForceFieldParams}};
use super::{frcmod_missing_params::MissingParams, parmchk_parse::{PARAMETERS, Position}, topology::invalid};

#[derive(Clone,Copy)]
enum Term { Bond, Angle, Proper, Improper }
...
fn score(from: &[&str], to: &[&str], term: Term) -> Option<(u8,f32,usize)> {
    ...
    for (i,(&a,&b)) in from.iter().zip(to).enumerate() {
        if b == "X" {
            if matches!(term,Term::Bond|Term::Angle) || (matches!(term,Term::Proper) && i != 0 && i != 3) { return None; }
            wildcard = true;
            if matches!(term,Term::Improper) { penalty += p.weight(if i == 2 { "WEIGHT_X3" } else { "WEIGHT_X" }); }
            continue;
        }
    ...
    if !equivalent {
        ...
        if matches!(term,Term::Proper) { penalty += p.pair_penalty([from[1],from[2]],[to[1],to[2]]); }
    }
    Some((stage,penalty,changed))
}

const PERMUTATIONS: [[usize;3];6] = [[0,1,3],[0,3,1],[1,0,3],[1,3,0],[3,0,1],[3,1,0]];
fn orientations<'a>(types: &[&'a str], term: Term) -> Vec<Vec<&'a str>> {
    if matches!(term,Term::Improper) {
        PERMUTATIONS.iter().map(|p| vec![types[p[0]],types[p[1]],types[2],types[p[2]]]).collect()
    } else { vec![types.to_vec(),types.iter().rev().copied().collect()] }
}

fn best<'a,T>(from: &[&str], table: impl Iterator<Item=(Vec<&'a str>, &'a T)>, term: Term) -> Option<(&'a T,String)> {
    // Lexical final tie break makes results independent of HashMap iteration.
    let mut best: Option<((u8,f32,Vec<usize>,Vec<&str>),&T)> = None;
    for (key,value) in table {
        for target in orientations(&key,term) {
            let Some((stage,penalty,_)) = score(from,&target,term) else { continue; };
            ...
            let order: Vec<_> = if matches!(term,Term::Proper) && stage == 1 {
                [1,2,0,3].iter().map(|&i| PARAMETERS.replacement_order(from[i],target[i])).collect()
            } else {
                from.iter().zip(&target).map(|(&a,&b)| PARAMETERS.replacement_order(a,b)).collect()
            };
            let rank = (stage,penalty,order,key.clone());
            if best.as_ref().is_none_or(|(old,_)| rank < *old) { best = Some((rank,value)); }
        }
    }
    best.map(|((_,penalty,_,key),value)| (value,format!("Estimated from {}; PARMCHK penalty {penalty:.3}",key.join("-"))))
}
...
pub fn assign_missing_params(atoms: &[AtomGeneric], adj: &[Vec<usize>], gaff2: &ForceFieldParams) -> io::Result<ForceFieldParams> {
    ...
    for key in missing.bond {
        ...
        let (source,comment) = best(&from,gaff2.bond.iter().map(|(k,v)| (vec![k.0.as_str(),k.1.as_str()],v)),Term::Bond)
            .ok_or_else(|| invalid(format!("No bond parameter for {key:?}")))?;
        let mut value = source.clone(); value.atom_types = key.clone(); value.comment = Some(comment);
        result.bond.insert(key,value);
    }
    for key in missing.angle {
        let from = [key.0.as_str(),key.1.as_str(),key.2.as_str()];
        let (source,comment) = best(&from,gaff2.angle.iter().map(|(k,v)| (vec![k.0.as_str(),k.1.as_str(),k.2.as_str()],v)),Term::Angle)
            .ok_or_else(|| invalid(format!("No angle parameter for {key:?}")))?;
        let mut value = source.clone(); value.atom_types = key.clone(); value.comment = Some(comment);
        result.angle.insert(key,value);
    }
    for (keys,table,term) in [(missing.dihedral,&gaff2.dihedral,Term::Proper),(missing.improper,&gaff2.improper,Term::Improper)] {
        for key in keys {
            let from = [key.0.as_str(),key.1.as_str(),key.2.as_str(),key.3.as_str()];
            let hit = best(&from,table.iter().filter(|(_,v)| !v.is_empty()).map(|(k,v)| (vec![k.0.as_str(),k.1.as_str(),k.2.as_str(),k.3.as_str()],v)),term);
            let values = if let Some((source,comment)) = hit {
                source.iter().map(|source| { let mut v = source.clone(); v.atom_types = key.clone(); v.comment = Some(comment.clone()); v }).collect()
            } else if matches!(term,Term::Improper) {
                // parmchk2's last resort, only for flagged three-coordinate hubs.
                vec![DihedralParams { atom_types: key.clone(), divider: 1, barrier_height: 1.1, phase: PI, periodicity: 2, comment: Some("Estimated: Amber default improper; no matching parameter".into()) }]
            } else { return Err(invalid(format!("No proper torsion parameter for {key:?}"))); };
            if matches!(term,Term::Proper) { result.dihedral.insert(key,values); } else { result.improper.insert(key,values); }
        }
    }
    Ok(result)
}
...
mod tests {
    ...
    fn improper_center_is_third_and_never_reversed() {
        ...
        for permutation in orientations(&from,Term::Improper) { assert_eq!(permutation[2],"c"); }
    }
    #[test]
    ...
    fn infers_missing_params_for_os_c3_nz_hn_chain() {
        ...
        let atoms = types.iter().enumerate().map(|(i,ty)| AtomGeneric { serial_number: i as u32+1, force_field_type: Some((*ty).into()), ..Default::default() }).collect::<Vec<_>>();
        let adj = vec![vec![1],vec![0,2],vec![1,3,4,5],vec![2],vec![2],vec![2]];
        let inferred = assign_missing_params(&atoms,&adj,params.small_mol.as_ref().unwrap()).unwrap();
        assert!(inferred.angle.contains_key(&("nz".into(),"c3".into(),"os".into())));
        assert!(inferred.dihedral.keys().any(|k| k.1 == "nz" && k.2 == "c3" || k.1 == "c3" && k.2 == "nz"));
