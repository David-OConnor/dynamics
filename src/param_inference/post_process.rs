...
pub(super) fn adjust_conjugated_types(env: &[AtomEnvData], types: &mut [String]) {
    fn partner(name: &str) -> Option<&'static str> {
        match name { "cc" => Some("cd"), "ce" => Some("cf"), "cg" => Some("ch"), "nc" => Some("nd"), "ne" => Some("nf"), "pc" => Some("pd"), "pe" => Some("pf"), "cp" => Some("cq"), _ => None }
    }
    ...
    for start in 0..types.len() {
        if signs[start].is_some() || partner(&types[start]).is_none() { continue; }
        let cp = types[start] == "cp";
        signs[start] = Some(false);
        let mut queue = VecDeque::from([start]);
        while let Some(i) = queue.pop_front() {
            for &(j, kind) in &env[i].bonds {
                if signs[j].is_some() || partner(&types[j]).is_none() || (types[j] == "cp") != cp { continue; }
                let flip = if cp { kind != BondKind::Single } else {
                    match kind {
    ...
    for (ty, sign) in types.iter_mut().zip(signs) {
        if sign == Some(true) { *ty = partner(ty).unwrap().to_owned(); }
    }
}
