...
use bio_files::{AtomGeneric, BondGeneric, amber_typedef::{AmberDef, AtomTypeDef, WildAtom}, md_params::ForceFieldParams};
use na_seq::Element;
...
impl AmberDefSet {
    pub fn new() -> io::Result<Self> {
        // bio_files 0.5.3 maps unsupported atomic numbers to Zinc. Exclude those
        // rows instead of accidentally assigning a metal the type of another element.
        fn parse(text: &str) -> io::Result<AmberDef> {
            let supported = text.lines().filter(|line| {
                let cols: Vec<_> = line.split_whitespace().collect();
                cols.first() != Some(&"ATD") || cols.get(3).and_then(|s| s.parse::<u8>().ok())
                    .is_none_or(|n| Element::from_atomic_number(n).is_ok())
            }).collect::<Vec<_>>().join("\n");
            AmberDef::new(&supported)
        }
        Ok(Self { abcg2: parse(DEF_ABCG2)?, gff2: parse(DEF_GFF2)? })
    }
}
static DEFAULT_DEFS: LazyLock<AmberDefSet> = LazyLock::new(|| AmberDefSet::new().expect("bundled Amber definitions"));
...
static PROPERTIES: LazyLock<HashMap<&'static str, Properties>> = LazyLock::new(|| {
    DEFAULT_DEFS.gff2.atomtypes.iter().map(|def| def.atomic_property.as_deref().unwrap_or("*"))
        .map(|text| (text,Properties::parse(text).expect("bundled property syntax"))).collect()
});
static ENVIRONMENTS: LazyLock<HashMap<&'static str, ChemEnvPattern>> = LazyLock::new(|| {
    DEFAULT_DEFS.gff2.atomtypes.iter().map(|def| def.chem_env.as_deref().unwrap_or("*"))
        .map(|text| (text,ChemEnvPattern::parse(text).expect("bundled environment syntax"))).collect()
});
...
fn is_elec_withdrawing_element(el: Element) -> bool {
    use Element::*;
    matches!(el, Oxygen | Nitrogen | Sulfur | Fluorine | Chlorine | Bromine | Iodine)
}

struct CompiledDef<'a> { def: &'a AtomTypeDef, properties: Cow<'static, Properties>, environment: Cow<'static, ChemEnvPattern> }
impl<'a> CompiledDef<'a> {
    fn new(def: &'a AtomTypeDef) -> io::Result<Self> {
        ...
        let properties = PROPERTIES.get(prop_text).map(Cow::Borrowed).or_else(|| Properties::parse(prop_text).map(Cow::Owned));
        let environment = ENVIRONMENTS.get(env_text).map(Cow::Borrowed).or_else(|| ChemEnvPattern::parse(env_text).map(Cow::Owned));
        match (properties, environment) {
            (Some(properties), Some(environment)) => Ok(Self { def, properties, environment }),
            _ => Err(topology::invalid(format!("Unsupported DEF syntax for {}", def.name))),
        }
    }
    fn matches(&self, idx: usize, atoms: &[AtomGeneric], env: &[AtomEnvData], wild: &[WildAtom]) -> bool {
        let def = self.def;
        let data = &env[idx];
        if def.element.is_some_and(|el| atoms[idx].element != el)
            || def.attached_atoms.is_some_and(|n| data.degree != n as usize)
            || def.attached_h.is_some_and(|n| data.num_attached_h != n as usize)
            || def.residue.as_deref().is_some_and(|res| res != "&") { return false; }
        if let Some(required) = def.electron_withdrawal_count {
            let Some(&(parent, _)) = data.bonds.first() else { return false; };
            let count = env[parent].bonds.iter().filter(|&&(nb,_)| is_elec_withdrawing_element(atoms[nb].element)).count();
...
pub fn matches_def(def: &AtomTypeDef, idx: usize, atoms: &[AtomGeneric], env_all: &[AtomEnvData], _bonds: &[BondGeneric], _adj: &[Vec<usize>]) -> bool {
    idx < atoms.len() && env_all.len() == atoms.len()
        && CompiledDef::new(def).is_ok_and(|rule| rule.matches(idx, atoms, env_all, &DEFAULT_DEFS.gff2.wildatoms))
}

fn assign_types(atoms: &[AtomGeneric], topology: &Topology, defs: &AmberDefSet) -> io::Result<Vec<String>> {
    ...
    let rules = defs.gff2.atomtypes.iter().map(CompiledDef::new).collect::<io::Result<Vec<_>>>()?;
    let mut result = (0..atoms.len()).map(|idx| rules.iter()
        .find(|rule| rule.matches(idx, atoms, &topology.env, &defs.gff2.wildatoms))
        .map_or_else(|| "du".to_owned(), |rule| rule.def.name.clone())).collect::<Vec<_>>();
...
pub fn try_find_ff_types(atoms: &[AtomGeneric], bonds: &[BondGeneric], defs: &AmberDefSet) -> io::Result<Vec<String>> {
...
pub fn update_small_mol_params(atoms: &mut [AtomGeneric], bonds: &[BondGeneric], adjacency_list: Option<&[Vec<usize>]>, gaff2: &ForceFieldParams) -> io::Result<ForceFieldParams> {
    let topology = Topology::new(atoms, bonds)?;
    if let Some(adj) = adjacency_list {
        if adj.len() != atoms.len() || adj.iter().zip(&topology.adj).any(|(given, expected)| {
            let mut given = given.clone(); given.sort_unstable(); given != *expected
    ...
    for (atom, ty) in staged.iter_mut().zip(types) {
        if ty.eq_ignore_ascii_case("du") { return Err(topology::invalid(format!("No GAFF2 type for atom {}", atom.serial_number))); }
    ...
    if charge.len() != atoms.len() || charge.iter().any(|v| !v.is_finite()) { return Err(io::Error::other("Invalid inferred charge vector")); }
