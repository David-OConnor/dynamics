...
impl Topology {
    pub fn new(atoms: &[AtomGeneric], bonds: &[BondGeneric]) -> io::Result<Self> {
        ...
        for bond in bonds {
            let (&a, &b) = indices
                .get(&bond.atom_0_sn)
                .zip(indices.get(&bond.atom_1_sn))
                .ok_or_else(|| invalid("Bond references an absent atom serial number"))?;
            if a == b || !seen.insert((a.min(b), a.max(b))) {
                return Err(invalid("Self bond or duplicate bond"));
            }
            let kind = match bond.bond_type {
                ...
                BondType::Aromatic => {
                    aromatic_edges.push(edges.len());
                    BondKind::AromaticSingle
                }
                _ => {
                    return Err(invalid(format!(
            ...
            adj[b].push(a);
            edges.push((a, b, kind));
            normalized_bonds.push(BondGeneric {
                atom_0_sn: a as u32 + 1,
                atom_1_sn: b as u32 + 1,
        ...
        let mut env: Vec<_> = adj
            .iter()
            .map(|neighbors| AtomEnvData {
                degree: neighbors.len(),
                num_attached_h: neighbors
                    .iter()
                    .filter(|&&i| atoms[i].element == Element::Hydrogen)
                    .count(),
                rings: [0; 11],
                aromatic: [0; 5],
                bonds: Vec::new(),
        ...
        for ring in &rings {
            for &i in ring {
                env[i].rings[ring.len()] += 1;
            }
        }
        let planar: Vec<i32> = atoms
            .iter()
            .enumerate()
            .map(|(i, atom)| {
                use Element::*;
        ...
        for ring in &rings {
            ...
            let outside_double = edges.iter().any(|&(a, b, kind)| {
                kind.has_property("db")
                    && ((ring.contains(&a) && env[b].rings.iter().sum::<usize>() == 0)
                        || (ring.contains(&b) && env[a].rings.iter().sum::<usize>() == 0))
            });
            let class = if sum == -2 * n {
                4
            } else if ring.iter().any(|&i| planar[i] < 0) {
                3
            } else if (n..=2 * n).contains(&sum) && outside_double {
                2
            } else if n == 6
                && sum == 12
                && ring.iter().all(|&i| {
                    !matches!(atoms[i].element, Element::Nitrogen | Element::Phosphorus)
                        || edges
                            .iter()
                            .any(|&(a, b, k)| (a == i || b == i) && k.has_property("db"))
                })
            {
                0
            } else if sum >= n + 3 {
        ...
        for &edge in &aromatic_edges {
            ...
            if !rings.iter().any(|r| {
                r.contains(&a)
                    && r.contains(&b)
                    && r.iter()
                        .all(|&i| env[i].aromatic[..3].iter().sum::<usize>() > 0)
            }) {
        ...
        for (a, b, kind) in &mut edges {
            if rings.iter().any(|r| {
                (5..=6).contains(&r.len())
                    ...
                    && r.contains(b)
                    && r.iter()
                        .all(|&i| env[i].aromatic[0] + env[i].aromatic[1] > 0)
            }) {
                *kind = match *kind {
                    BondKind::Single => BondKind::AromaticSingle,
                    BondKind::Double => BondKind::AromaticDouble,
                    other => other,
                };
            }
        }
        for (a, b, kind) in edges {
            env[a].bonds.push((b, kind));
            env[b].bonds.push((a, kind));
        }
        for data in &mut env {
            data.bonds.sort_unstable_by_key(|&(i, _)| i);
        }
        Ok(Self {
            env,
            adj,
...
fn find_rings(atoms: &[AtomGeneric], adj: &[Vec<usize>]) -> Vec<Vec<usize>> {
    use Element::*;
    let eligible: Vec<_> = atoms
        .iter()
        .enumerate()
        .map(|(i, a)| match a.element {
            Carbon => adj[i].len() > 2,
    ...
    fn visit(
        ...
        for &next in &adj[last] {
            if next <= start || !eligible[next] || path.contains(&next) {
                continue;
            }
            let closes = path.len() >= 2 && adj[next].contains(&start);
            if path[1..path.len() - 1]
                .iter()
                .any(|v| adj[next].contains(v))
            {
                continue;
            }
            if closes {
    ...
    for start in 0..atoms.len() {
        ...
        for &next in &adj[start] {
            if next > start && eligible[next] {
                visit(&mut vec![start, next], adj, &eligible, &mut rings);
            }
        }
    }
    rings
...
fn kekulize(
    atoms: &[AtomGeneric],
    adj: &[Vec<usize>],
    edges: &mut [(usize, usize, BondKind)],
    aromatic: &[usize],
) -> io::Result<()> {
    ...
    for &edge in aromatic {
        ...
        for i in [a, b] {
            ...
            if edges.iter().any(|&(x, y, k)| {
                (x == i || y == i) && matches!(k, BondKind::Double | BondKind::Triple)
            }) {
                required[i] = false;
            }
        }
    ...
    fn solve(
        required: &mut [bool],
        edges: &[(usize, usize, BondKind)],
        aromatic: &[usize],
        chosen: &mut Vec<usize>,
    ) -> bool {
        let Some(i) = required.iter().position(|&r| r) else {
            return true;
        };
        required[i] = false;
        for &e in aromatic {
            ...
            required[j] = false;
            chosen.push(e);
            if solve(required, edges, aromatic, chosen) {
                return true;
            }
            chosen.pop();
            required[j] = true;
    ...
    if !solve(&mut required, edges, aromatic, &mut chosen) {
        return Err(invalid(
            "Cannot resolve aromatic bond orders; supply explicit single/double (Kekule) bonds",
        ));
    }
    for e in chosen {
        edges[e].2 = BondKind::AromaticDouble;
    }
    Ok(())
}
