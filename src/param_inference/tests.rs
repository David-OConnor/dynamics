...
fn fixtures() -> Vec<(
    ...
    include_str!("fixtures/amber_atomtypes.tsv")
        .lines()
        ...
        .map(|line| {
            ...
            let atoms = cols[2]
                .split(',')
                .enumerate()
                .map(|(i, el)| AtomGeneric {
                    serial_number: i as u32 + 1,
                    element: Element::from_letter(el).unwrap(),
                    ..Default::default()
                })
                .collect();
            let bonds = cols[3]
            ...
            (
                ...
                atoms,
                bonds,
                cols[4].split(',').map(str::to_owned).collect(),
            )
        })
        .collect()
}
...
#[test]
fn high_level_charge_inference_accepts_arbitrary_serial_numbers() {
    let (_, mut atoms, mut bonds, _) = fixtures()
        .into_iter()
        .find(|(name, _, _, _)| *name == "methane")
        .unwrap();
    let parameters = crate::params::FfParamSet::new_amber().unwrap();
    let gaff = parameters.small_mol.as_ref().unwrap();
    let mut expected = atoms.clone();
    update_small_mol_params(&mut expected, &bonds, None, gaff).unwrap();
    for atom in &mut atoms {
        atom.serial_number = (atom.serial_number - 1) * 17;
    }
    for bond in &mut bonds {
        bond.atom_0_sn = (bond.atom_0_sn - 1) * 17;
        bond.atom_1_sn = (bond.atom_1_sn - 1) * 17;
    }
    update_small_mol_params(&mut atoms, &bonds, None, gaff).unwrap();
    for (actual, expected) in atoms.iter().zip(&expected) {
        assert_eq!(actual.force_field_type, expected.force_field_type);
        assert!(actual.partial_charge.unwrap().is_finite());
        assert!((actual.partial_charge.unwrap() - expected.partial_charge.unwrap()).abs() < 1e-6);
    }
    assert_eq!(atoms[0].serial_number, 0);
}
...
fn serial_numbers_and_bond_direction_do_not_change_types() {
    for (name, mut atoms, mut bonds, _) in fixtures() {
        ...
        for atom in &mut atoms {
            atom.serial_number = atom.serial_number * 17 + 42;
        }
        for bond in &mut bonds {
        ...
        bonds.reverse();
        assert_eq!(
            try_find_ff_types(&atoms, &bonds, &DEFAULT_DEFS).unwrap(),
            expected,
            "{name}"
        );
    }
...
fn invalid_graph_returns_error_and_legacy_api_returns_dummies() {
    ...
    assert_eq!(
        find_ff_types(&atoms, &bonds, &DEFAULT_DEFS),
        vec!["du"; atoms.len()]
    );
...
fn def_grammar_checks_counts_branches_and_parent_bonds() {
    let (_, atoms, bonds, _) = fixtures()
        .into_iter()
        .find(|(n, _, _, _)| *n == "acetamide")
        .unwrap();
    let topo = Topology::new(&atoms, &bonds).unwrap();
    let matches = |text, idx| {
        ChemEnvPattern::parse(text).unwrap().matches(
            idx,
            &atoms,
            &topo.env,
            &DEFAULT_DEFS.gff2.wildatoms,
        )
    };
    ...
    assert!(
        Properties::parse("[0RG,1DB,2SB]")
            .unwrap()
            .matches(1, None, &topo.env)
    );
    assert!(
        Properties::parse("[RG3.RG4,DB]")
            .unwrap()
            .matches(1, None, &topo.env)
            == false
    );
...
fn all_oracle_molecules_have_bonded_parameters() {
    ...
    for (name, mut atoms, bonds, types) in fixtures() {
        for (atom, ty) in atoms.iter_mut().zip(types) {
            atom.force_field_type = Some(ty);
        }
        let topo = Topology::new(&atoms, &bonds).unwrap();
        if let Err(e) = assign_missing_params(&atoms, &topo.adj, params.small_mol.as_ref().unwrap())
        {
...
fn compare_parmchk2_oracle() {
    ...
    for block in include_str!("fixtures/amber_frcmod.txt")
        .split("@@")
        .skip(1)
    {
        let (name, text) = block.split_once('\n').unwrap();
        let name = name.trim();
        let reference = ForceFieldParams::from_frcmod(text).unwrap();
        let (_, atoms, bonds, types) = fixtures.iter().find(|(n, _, _, _)| *n == name).unwrap();
        let mut atoms = atoms.clone();
        for (a, t) in atoms.iter_mut().zip(types) {
            a.force_field_type = Some(t.clone());
        }
        let topo = Topology::new(&atoms, bonds).unwrap();
        ...
        for (key, expected) in &reference.bond {
            let actual = ours
                .get_bond(key, false)
                .or_else(|| gaff.get_bond(key, false))
                .unwrap();
            if (actual.k_b - expected.k_b).abs() > 0.01
                || (actual.r_0 - expected.r_0).abs() > 0.0001
            {
                differences.push(format!(
                    "{name} bond {key:?}: {} vs {} ({:?})",
                    actual.k_b, expected.k_b, expected.comment
                ));
            } else {
                matched += 1;
            }
        ...
        for (key, expected) in &reference.angle {
            let actual = ours
                .get_valence_angle(key, false)
                .or_else(|| gaff.get_valence_angle(key, false))
                .unwrap();
            if (actual.k - expected.k).abs() > 0.01
                || (actual.theta_0 - expected.theta_0).abs() > 0.0001
            {
                differences.push(format!(
                    "{name} angle {key:?}: {} vs {} ({:?})",
                    actual.k, expected.k, expected.comment
                ));
            } else {
                matched += 1;
            }
        ...
        for (proper, table) in [(true, &reference.dihedral), (false, &reference.improper)] {
            for (key, reference_terms) in table {
                ...
                if proper
                    && matches!(name, "aziridine" | "n_methylaziridine" | "epoxide")
                    && key.0 == "cx"
                    && key.1 == "cx"
                    && key.3 == "cx"
                {
                ...
                if !proper
                    && ((matches!(name, "acetone" | "methylacetate" | "aspirin")
                        && key.2 == "c"
                        && [key.0.as_str(), key.1.as_str(), key.3.as_str()].contains(&"o"))
                        || (matches!(name, "pyrimidine" | "adenine" | "triazine")
                            && key.0 == "h5"
                            && key.1 == "nb"
                            && key.2 == "ca"
                            && key.3 == "nb"))
                ...
                let actual = ours
                    .get_dihedral(key, proper, false)
                    .or_else(|| gaff.get_dihedral(key, proper, true));
                if let Some(actual) = actual {
                    if actual.len() != expected.len()
                        || actual.iter().zip(&expected).any(|(a, e)| {
                            a.divider != e.divider
                                || a.periodicity != e.periodicity
                                || (a.barrier_height - e.barrier_height).abs() > 0.011
                                || (a.phase - e.phase).abs() > 0.001
                        })
                    {
                        differences.push(format!(
                            "{name} torsion {proper} {key:?}: {:?} vs {:?} ({:?})",
                            actual.iter().map(|x| x.barrier_height).collect::<Vec<_>>(),
                            expected
                                .iter()
                                .map(|x| x.barrier_height)
                                .collect::<Vec<_>>(),
                            expected[0].comment
                        ));
                    } else {
                    ...
                    differences.push(format!("{name} torsion {proper} {key:?}: missing"));
                }
            }
        }
    }
...
fn rings_are_counted_once_and_fused_perimeters_are_not_rings() {
    for (name, atoms, bonds, _) in fixtures() {
        ...
        if name == "cyclononane" {
            assert_eq!(topo.env[0].rings[9], 1);
        }
        if name == "cyclodecane" {
            assert_eq!(topo.env[0].rings[10], 1);
        }
    }
}
...
fn unsupported_atomic_numbers_do_not_alias_to_zinc() {
    let atom = AtomGeneric {
        serial_number: 0,
        element: Element::Zinc,
        ..Default::default()
    };
    assert_eq!(
        try_find_ff_types(&[atom], &[], &DEFAULT_DEFS).unwrap(),
        ["Zn"]
    );
}
...
fn malformed_and_unsupported_inputs_are_rejected_before_mutation() {
    ...
    assert!(
        update_small_mol_params(&mut atoms, &bonds, None, &ForceFieldParams::default()).is_err()
    );
    ...
    assert!(
        update_small_mol_params(&mut atoms, &bonds, Some(&[]), &ForceFieldParams::default())
            .is_err()
    );
    assert_eq!(atoms[0].force_field_type.as_deref(), Some("original"));
    assert!(
...
fn atom_order_does_not_change_chemical_types() {
    ...
    for (name, mut atoms, bonds, _) in fixtures() {
        ...
        assert_eq!(
            before.iter().map(|s| base(s)).collect::<Vec<_>>(),
            after.iter().map(|s| base(s)).collect::<Vec<_>>(),
            "{name}"
        );
    }
}
...
fn conjugated_pairs_preserve_single_and_flip_multiple_bonds() {
    fn sign(ty: &str) -> Option<bool> {
        match ty {
            "cc" | "ce" | "cg" | "nc" | "ne" | "pc" | "pe" => Some(false),
            "cd" | "cf" | "ch" | "nd" | "nf" | "pd" | "pf" => Some(true),
            _ => None,
        }
    }
    for (name, atoms, bonds, _) in fixtures() {
        ...
        for (i, env) in topo.env.iter().enumerate() {
            for &(j, kind) in &env.bonds {
                if let (Some(a), Some(b)) = (sign(&types[i]), sign(&types[j])) {
                    if kind.has_property("sb") {
                        assert_eq!(a, b, "{name}: single {i}-{j}");
                    }
                    if kind.has_property("db") || kind.has_property("tb") {
                        assert_ne!(a, b, "{name}: multiple {i}-{j}");
                    }
                }
            }
        }
